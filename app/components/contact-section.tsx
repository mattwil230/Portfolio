      ...
      <div className="mx-auto max-w-6xl px-6 py-20 sm:py-24">
        <FadeInSection className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            ...
            <a
              href={`mailto:${email}`}
              className="group mt-8 inline-flex items-center gap-3 text-2xl font-semibold tracking-tight text-zinc-950 transition-colors duration-200 hover:text-blue-700 sm:text-3xl"
            >
              {email}
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-3">
            {links.map((link) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group inline-flex items-center gap-2 text-sm text-zinc-600 transition-colors duration-200 hover:text-zinc-950"
              >
                {link.label}
                <span className="h-px w-0 bg-blue-600 transition-all duration-200 group-hover:w-4" />
              </a>
            ))}
          </div>
        </FadeInSection>
